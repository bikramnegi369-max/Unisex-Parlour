...
export default function EmployeeDetailsPage({ employeeId }: EmployeeDetailsPageProps) {
    ...
    assignMultipleServicesMutation.mutate(
      { id: employeeId, serviceIds: selectedServiceIds },
      {
        onSuccess: () => {
          ...
          toast.success(`${selectedServiceIds.length} service capabilities assigned successfully.`);
        },
        onError: (err: Error) => {
  ...
  const assignedBranchIds = useMemo(() => (staffBranches || []).map((sb) => sb.branchId?._id), [staffBranches]);
  const assignableBranches = useMemo(() => (allBranches || []).filter((b) => !assignedBranchIds.includes(b.id)), [allBranches, assignedBranchIds]);
  const filteredBranches = useMemo(
    () => assignableBranches.filter((b) => b.name.toLowerCase().includes(branchSearchQuery.toLowerCase())),
    [assignableBranches, branchSearchQuery]
  );

  const assignedServiceIds = useMemo(() => (staffServices || []).map((ss) => ss.serviceId?._id), [staffServices]);
  const assignableServices = useMemo(() => allServices.filter((s) => !assignedServiceIds.includes(s.id)), [allServices, assignedServiceIds]);
  const filteredServices = useMemo(
    () => assignableServices.filter((s) => s.name.toLowerCase().includes(serviceSearchQuery.toLowerCase())),
  ...
  const tabs: ProfileTabItem[] = useMemo(
    () => [
      ...
      { id: "branches", label: "Branches", icon: <Building2 size={15} />, badge: branchCount },
      { id: "services", label: "Service Capabilities", icon: <Scissors size={15} />, badge: serviceCount },
      { id: "account", label: "User Account", icon: <UserCog size={15} />, badge: hasLinkedAccount ? "Linked" : "Unlinked" },
    ],
            ...
            <div className="bg-card border border-border/80 rounded-2xl p-6 shadow-sm space-y-6 min-w-0">
              ...
              {canAssignBranch && assignableBranches.length > 0 && (
                <div className="p-4 rounded-2xl border border-border bg-muted/10 max-w-2xl space-y-4 text-left shadow-2xs">
                  <div className="space-y-1 relative">
                    <label htmlFor="branch-search-combobox" className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider block">
                      ...
                      {showBranchDropdown && branchSearchQuery && (
                        <div className="absolute z-20 w-full bg-card border border-border shadow-xl rounded-xl mt-1.5 max-h-52 overflow-y-auto p-1">
                          {filteredBranches.length === 0 ? (
                            <p className="text-xs text-muted-foreground p-3 text-center">No matching branches found.</p>
                          ) : (
                            filteredBranches.map((b) => (
                              ...
                              >
                                ...
                                {b.address && <span className="text-[10px] text-muted-foreground truncate">{b.address}</span>}
                              </button>
                            ))
                          )}
                        </div>
              ...
              {isLoadingBranches || isLoadingAllBranches ? (
                ...
                <div className="flex flex-col items-center justify-center py-10 text-center border border-dashed border-border rounded-2xl">
                  ...
                  <p className="text-sm font-bold text-foreground">No branches assigned</p>
            ...
            <div className="bg-card border border-border/80 rounded-2xl p-6 shadow-sm space-y-6 min-w-0">
                ...
                <div className="p-5 rounded-2xl border border-purple-500/20 bg-purple-50/20 dark:bg-purple-950/10 max-w-2xl space-y-4 text-left shadow-2xs">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground">Select one or multiple services from the catalog to assign simultaneously.</p>
                    </div>
                    ...
                    <div className="max-h-60 overflow-y-auto border border-border/80 rounded-xl bg-card p-1 divide-y divide-border/40 shadow-inner">
                      {filteredServices.length === 0 ? (
                        <p className="text-xs text-muted-foreground p-4 text-center">
                          {serviceSearchQuery ? `No services matching "${serviceSearchQuery}".` : "All catalog services are already assigned!"}
                        </p>
                      ) : (
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-center justify-between w-full p-2.5 text-left text-xs transition-colors rounded-lg ${
                                isChecked ? "bg-purple-500/10 text-foreground font-semibold" : "hover:bg-muted/70 text-foreground"
                              }`}
                            >
                              <div className="flex items-center gap-2.5 min-w-0">
                                ...
                                <div className="min-w-0">
                                  ...
                                  <p className="text-[10px] text-muted-foreground">
                                    {service.duration} mins • Base Price: ₹{service.pricing?.basePrice}
                                  </p>
                                </div>
                              </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-purple-500/10">
                    ...
                    <Button
                      ...
                      disabled={selectedServiceIds.length === 0 || assignMultipleServicesMutation.isPending}
              ...
              {isLoadingServices || isLoadingAllServices ? (
                ...
                <div className="flex flex-col items-center justify-center py-10 text-center border border-dashed border-border rounded-2xl">
                  ...
                  <p className="text-sm font-bold text-foreground">No service capabilities assigned</p>
              ...
              <div className="flex items-center justify-between gap-4 border-b border-border/80 pb-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Manage authentication account connection for staff portal access.</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="rounded-xl border border-border/80 bg-card p-4 space-y-3">
                    <div className="flex items-start gap-3">
                      <div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 text-primary font-bold text-lg shrink-0">
                        {(linkedUser?.name || employee.name || "U").charAt(0).toUpperCase()}
                      </div>
                      <div className="min-w-0 space-y-1">
                        <h4 className="font-bold text-sm text-foreground">{linkedUser?.name || employee.name}</h4>
                        {linkedUser?.username && (
                          <p className="text-xs text-muted-foreground font-mono">@{linkedUser.username}</p>
                        )}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 pt-1 text-xs text-muted-foreground">
                          {(linkedUser?.email || employee.email) && (
                            <span className="flex items-center gap-1.5 truncate">
                              ...
                              <span className="truncate">{linkedUser?.email || employee.email}</span>
                            </span>
                          )}
                ...
                <div className="space-y-6 text-left max-w-2xl">
                  <div className="flex items-start gap-3 p-4 rounded-xl border border-amber-500/30 bg-amber-50/40 dark:bg-amber-950/20 text-amber-900 dark:text-amber-300">
                    ...
                    <div className="text-xs space-y-1">
                      ...
                      <p>
                        This employee record is currently unlinked. Linking a user account grants this employee access to sign in to the parlour management application.
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="space-y-4 p-5 rounded-2xl border border-border bg-muted/10 shadow-2xs">
                      ...
                      <div className="flex justify-end pt-2">
                        <Button
                          ...
                          disabled={!selectedUserSummary?.id || linkUserMutation.isPending}
