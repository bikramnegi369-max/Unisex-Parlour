...
export function CustomerSearch({ value, onChange, placeholder, isLoading }: CustomerSearchProps) {
  const inputRef = useRef<HTMLInputElement>(null);

  // Focus input when user presses the '/' shortcut key
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Focus if '/' is pressed and no input/textarea/editable is currently focused
      if (
        e.key === "/" &&
  ...
  return (
    <div className="relative w-full md:max-w-md select-none group">
      ...
      <Input
        ...
        placeholder={placeholder || "Search customers by name, phone, or email..."}
        className="pl-9 pr-10 h-9 text-xs rounded-lg transition-all"
      />

      {/* Right-aligned Utility Controls (Loading spinner, Clear button, or Keyboard Shortcut Badge) */}
